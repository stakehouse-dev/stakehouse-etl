...
STAKEHOUSE_GRAPH_URL = "https://api.thegraph.com/subgraphs/name/stakehouse-dev/stakehouse-protocol"
BEACONCHAIN_SLOT_BLOCK_NUMBER= 1
STAKEHOUSE_GRAPH_INDEX_V2_URL= "https://api.thegraph.com/subgraphs/name/stakehouse-dev/lsd"
STAKEHOUSE_LIQUID_INDEX_GRAPH_URL = ""
SUPPORT_V1_INDEX= 0


def fetch_all_bls_keys_from_subgraph(): # TODO: ADJUST TO SUPPORT FOR > 1000 knots
    q = """
        {
...
def user_validators(user):
    ...
    savETHIndex2= resp2.get('data').get('protectedBatches', None)

    validators= {validator['lsdValidator']['id']: (sum([float(i['amount']) for i in validator['liquidityProviders']]) / 10 ** 18) for validator in savETHIndex2 if len(validator['lsdValidator'])> 0}

    return validators

def user_validator_share(user):
    q = """
           {
        protectedBatches(first: 1000, where: {
            vaultLPToken_:{
    ...
    resp2 = requests.post(STAKEHOUSE_GRAPH_INDEX_V2_URL, json={'query': q}).json()
    savETHIndex2= resp2.get('data').get('protectedBatches', None)
    validators = [i['liquidityProviders'] for i in savETHIndex2 if len(i['liquidityProviders']) > 0]
    return sum([int(i[0]['amount']) for i in validators])
...
def user_mev_batch(user):
    ...
    validator_slot= {validator['blsPublicKey']: (sum([float(i['amount']) for i in validator['liquidityProviders']])/ 10 ** 18) for validator in feesAndMevBatches}
    indexes= set([validator['lsdValidator']['liquidStakingManager'] for validator in feesAndMevBatches])

    payouts = sum([float(validator['amount']) for validator in resp2.get('data').get('payouts', [])]) / (10 ** 18)
