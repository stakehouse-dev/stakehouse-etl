...
def withdrawals_slot(slot):
    ...
    if not execution_payload:
        ...
        return validator, withdrawal_recipient, validator_withdrawal_index, status_validator
...
def data_handler(skip: int):
    ...
    for slot in slots:
        ...
        withdrawals, withdrawal_recipient, validator_withdrawal_index, status_withdrawals = withdrawals_slot(
            slot)
        print(f"Processing Slot {slot} in Run {int(skip / SLOTS_PROCESSOR)}, {status_withdrawals}")
...
for slot_skip in range(5000, 12137):
