...
def data_handler(event):
    ...
    for epoch_number in record_index:
        ...
        print("\n")
        print(f'\tEpoch num {epoch_number} and BLS keys')
        print(bls_keys)
...
def validator_backfiller_queue_handler():
    ...
    try:
        ...
        for bls_key_group in bls_key_groups:
            ...
            if len([bls_key for bls_key in list(epochs.keys()) if len(epochs[bls_key])!= 0]) == 0:
                print(f'\tNo BLS Key Has Epochs After {last_epoch} for BLS Keys Skipping {len(bls_key_group)} BLS Keys')
            for bls_key in [bls_key for bls_key in list(epochs.keys()) if len(epochs[bls_key])!= 0]:
                ...
                if len(epochs_to_update)== 0:
                    print(f'\tValidator {bls_key} No Missed Epoch, Skipping It')
                else:
                    print(f'\tValidator {bls_key}')
                    print(epochs_to_update)
                    print("\n")
                messages= []
        ...
        if len(MAX_LAST_EPOCH)> 0:
            max_epoch = sorted(MAX_LAST_EPOCH)[0]
...
#schedule.every(1).minutes.do(validator_backfiller_queue_handler)
#while 1:
